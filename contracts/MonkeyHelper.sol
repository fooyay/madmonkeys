...
contract MonkeyHelper is MonkeyFeeding {
    ...
    function changeName(uint _monkeyId, string calldata _newName) external aboveLevel(2, _monkeyId) {
        require(msg.sender == monkeyToOwner[_monkeyId]);
        monkeys[_monkeyId].name = _newName;
    }

    function changeDna(uint _monkeyId, uint _newDna ) external aboveLevel(20, _monkeyId) {
        require(msg.sender == monkeyToOwner[_monkeyId]);
        monkeys[_monkeyId].dna = _newDna;
    }

    function getMonkeysByOwner(address _owner) external view returns(uint[] memory) {
